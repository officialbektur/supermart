async function asyncProductsRec() {
	if (!document.querySelector(".moreinfo-tovar__alsorecommend .swiper-wrapper")) {
		return false;
	}
	try {
		...
		if (response.ok) {
			...
			let menuHTML = buildHTMLRec(result);
			content.innerHTML = '';

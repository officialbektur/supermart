...
<section class="content">
	<div class="content__container">
		<h2 class="_title">
			<?php
				if (breadcrumb($_GET["category"])) {
					$breadcrumbs = preg_replace("#(.+)?<a.+>(.+)</a>$#", "$1$2", breadcrumb($_GET["category"]));
					echo $title . " / ". $breadcrumbs;
				} else {
					echo $title;
				}
		...
		<div class="content__body">
			<?php
				$ids = cats_id(get_cat(), $_GET["category"]);
				$ids = !$ids ? $_GET["category"]: rtrim($ids, ",");
				$allProducts = get_product_cat($ids);
				if (mysqli_num_rows($allProducts)) {
				...
				<div class="product__block product-block">
					...
					<div class="product-block__info product-block-info">
						...
						<div class="product-block-info__price product-block-info-price">
							<?php
								if (isset($allProduct["priceOld"])) {
									if ($allProduct["priceOld"] > 0) {
							...
							<div class="product-block-info-price__now product-block-info-price-now">
								<span class="product-block-info-price-now__number">
									<?php
										if (isset($allProduct["priceNow"])) {
											echo getbrosocrat($allProduct["priceNow"]);
										}
									?>
								</span>
			...
			<?php
				endforeach;
					...
					echo "<p><b>Нету товаров под таким категорий!</b></p>";
				}
			?>
		</div>
	</div>
